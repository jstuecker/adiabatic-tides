...
def test_sample_radii_fixed(profile, embed_plot):
    ...
    rs = at.mathtools.sample_radii(ri, prof.m_of_r(ri), 1000000, rmax=rmax)
...
def test_sample_radii_adaptive(profile, embed_plot):
    ...
    rs,ms = at.mathtools.sample_rimi_from_density(ri, prof.density(ri), 1000000, weights=weights )
...
#     weights = 1. / (prof.density(ri)*ri**2 * ri)
#     rs,ms = at.mathtools.sample_rimi_from_density(ri, prof.density(ri), 200000, weights=weights)
#     Es = at.mathtools.sample_conditional_energy_adaptive_batched(prof.potential(rs), prof.f_of_e, nintegrate=400)
...
def test_sample_and_integrate(profile, embed_plot):
    ...
    tmax = prof.tcirc(rps)
    # rs, vrs = at.mathtools.integrate_radial_orbits(prof.accr, rs, vrs, Ls, tmax, nsteps=1000)
    # rs = np.abs(rs)

    # Now integrate and measure time-averaged mass profile
    # (The time-averaging helps enhance the statistics at same costs)
    rho, n = 0, 0
    for rs, vrs in at.mathtools.integrate_radial_orbits_with_snaps(prof.accr, rs, vrs, Ls, tmax, nsnaps=100, nsteps_per_snap=10):
        rhoinew, minew = at.mathtools.get_mass_profile(rs, ms, rbins)
        rho += rhoinew
        n += 1
    rho /= n
